#!/usr/bin/env python3
"""
AA.com Extended Wait Scraper
Uses Scrapling with extended wait times for JavaScript to fully load flight results
This avoids browser automation detection while still getting the data
"""
...
def search_flights_with_extended_wait():
    """
    Search for flights using Scrapling with extended wait for JavaScript rendering
    """
    print("✈️  Extended Wait Flight Search...")
    print("🎯 Target: LAX → JFK on December 15, 2025")
    print("⏳ Using extended wait times for JavaScript to fully load")
    print("🛡️  Avoiding browser automation detection")
    ...
    try:
        ...
        homepage = StealthyFetcher.fetch(
            ...
            headless=True,  # Use headless to avoid detection
            solve_cloudflare=True,
            humanize=3.0,  # Longer human-like delays
            geoip=True,
            os_randomize=True,
            disable_ads=True,
            google_search=True,
        ...
        print("⏳ This will wait up to 2 minutes for Angular app to fully load...")
        
        def extended_wait_for_content(page):
            print("⏳ Starting extended wait for JavaScript content...")
            ...
            try:
                if hasattr(page, 'css'):
                    ...
                    # Extended wait for API calls to complete
                    if len(flight_elements) < 10:
                        print("⏳ Content still loading, waiting 30 more seconds...")
                        time.sleep(30)
                    ...
                    # Final extended wait for any remaining API calls
                    print("⏳ Final extended wait for API calls to complete...")
                    time.sleep(60)  # Wait 1 full minute
        ...
        search_page = StealthyFetcher.fetch(
            ...
            block_webrtc=True,
            allow_webgl=False,
            page_action=extended_wait_for_content
        )
        
        print(f"\n📍 Final URL: {search_page.url}")
        
        # Step 4: Extract flight data with comprehensive analysis
        flight_data = extract_comprehensive_flight_data(search_page, search_url)
...
def extract_comprehensive_flight_data(page, url):
    """
    Extract comprehensive flight data with multiple analysis methods
    """
    print("\n💰 Extracting comprehensive flight data...")
    ...
    flight_data = {
        ...
        "extraction_method": "extended_wait_comprehensive_analysis",
        "search_url": url,
    ...
    try:
                    ...
                    for i, element in enumerate(elements):
                        try:
                            ...
                            if len(text_content) > 20:
                                # Parse flight data
                                flight_info = parse_enhanced_flight_data(text_content, i, selector)
                                if flight_info:
        ...
        if not flights_found:
            ...
            try:
                ...
                for element in all_elements[:1000]:  # Check first 1000 elements
                    try:
                ...
                for i, (element, text) in enumerate(potential_flights[:50]):  # Limit to first 50
                    try:
                        flight_info = parse_enhanced_flight_data(text, i, "comprehensive_analysis")
                        if flight_info:
                            flight_data["flights"].append(flight_info)
                    except Exception as e:
...
def parse_enhanced_flight_data(text, index, selector):
    """
    Enhanced parsing of flight data with better regex patterns
    """
    try:
        ...
        if (points_required or cash_price_usd or 
            ...
            return {
                ...
                "raw_text": text[:400],  # First 400 chars for debugging
...
def save_extended_wait_results(flight_data):
    """
    Save the extended wait results
    """
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"aa_extended_wait_results_{timestamp}.json"
    
    try:
        with open(filename, 'w', encoding='utf-8') as f:
            json.dump(flight_data, f, indent=2, ensure_ascii=False)
        
        print(f"\n💾 Extended wait results saved to: {filename}")
        
        # Save detailed summary
        summary_filename = f"aa_extended_wait_summary_{timestamp}.txt"
        with open(summary_filename, 'w', encoding='utf-8') as f:
            f.write(f"AA.com Extended Wait Flight Search Results\n")
            f.write(f"==========================================\n")
            ...
            if page_analysis:
                ...
                f.write(f"  Divs: {page_analysis.get('divs_count', 0)}\n")
                f.write(f"  Spans: {page_analysis.get('spans_count', 0)}\n")
...
def main():
    """
    Main function to run the extended wait search
    """
    print("=" * 80)
    print("⏳ AA.com Extended Wait Flight Search")
    print("=" * 80)
    print("🎯 Target: LAX → JFK on December 15, 2025")
    print("⏳ Using extended wait times for JavaScript to fully load")
    print("🛡️  Avoiding browser automation detection")
    ...
    # Perform extended wait flight search
    flight_data = search_flights_with_extended_wait()
    
    if flight_data:
        # Save results
        save_extended_wait_results(flight_data)
        
        print("\n" + "=" * 80)
        print("🎉 Extended wait search completed!")
        print("=" * 80)
        ...
        if page_analysis:
            print(f"\n📊 Page Analysis:")
            print(f"  Total elements: {page_analysis.get('total_elements', 0)}")
            print(f"  Links: {page_analysis.get('links_count', 0)}")
            print(f"  Images: {page_analysis.get('images_count', 0)}")
            print(f"  Forms: {page_analysis.get('forms_count', 0)}")
            print(f"  Inputs: {page_analysis.get('inputs_count', 0)}")
            print(f"  Buttons: {page_analysis.get('buttons_count', 0)}")
        ...
        if flights:
            ...
            print("   - Need for even longer wait times")
            print("   - Different selectors needed")
    else:
        print("\n" + "=" * 80)
        print("❌ Extended wait search failed!")
        print("=" * 80)
